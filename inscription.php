...
if(isset($_POST["inscription"]))  
     {  
          ...
          {
               $userExists = $connect->loginExiste($_POST['login']);
               if(!$userExists) {
                $count_request = "SELECT count(*) FROM utilisateur";
                $count = $connect->compteResultat($count_request);
                $count = $count+1;
                $connect->inscritUtilisateur($_POST['login'], $_POST['motDePasse'], $count);
                $user = new Utilisateur($connect, $count);               
                echo $count;
                $_SESSION['userInscrit'] = true;
                header("location:login.php");          
               }
               else {  
                    $message = '<div class="alerteConnexion">Un utilisateur de ce nom existe déjà</div>';  
               } 
          } 
     }
}  
 ...
 <html>  
      ...
      <body>   
           <div class="container" style="width:500px;">
            ...
            <a href="login.php">Connexion</a> | <a href="dashboard.php">Déjà membre?</a>   
                <?php  
                if(isset($message))  
                {  
                ...
                ?>  
                <h3 align=""> Veuillez entrer vos information de connexion</h3><br />  
                <form method="post" class="formulaire">    
                     ...
                     <br />  
                     <input type="submit" name="inscription" class="btn btn-info" value="Inscription" />  
                </form>  
           </div>  
           <br />  
      </body> 
